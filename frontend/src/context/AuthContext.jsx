...
export function AuthProvider({ children }) {
  ...
  useEffect(() => { const token = getToken(); if (token) fetchMe(token); else setLoading(false) }, [])

  async function fetchMe(token, isRetry = false) {
    try {
      ...
      if (!isRetry) { setTimeout(() => fetchMe(token, true), 4000) }
