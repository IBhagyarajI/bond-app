...
const { notifyCheckIn } = require("../notifications");
...
async function askGroq(prompt) {
  ...
  const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: "llama-3.1-8b-instant", messages: [{ role: "user", content: prompt }], max_tokens: 500, temperature: 0.8 }),
  });
...
router.post("/checkin", async (req, res) => {
  try {
    ...
    const prompt = `You are a warm friendship companion for "Bond".
User: ${user.name}, Friend: ${friend ? friend.name : "not bonded"}
Bonded: ${bondDays} days, Memories: ${memoriesCount}, Bucket done: ${bucketCount}
Mood: ${moodLabels[mood] || mood}/5, Message: "${message || "No message"}"
Write a warm personal response in 2-3 short paragraphs. Acknowledge mood, say something meaningful about friendship, give one small actionable suggestion.`;
    ...
    if (user.friend_id) {
      ...
      // Notify friend
      notifyCheckIn(client, req.userId, mood).catch(() => {});
    }
...
${user.name} wants to support ${name} going through: "${situation}"
Give 4-5 specific, actionable numbered ways to show up. Be warm, practical, avoid clichés.`;
    const aiText = await askGroq(prompt);
    res.json({ response: aiText });
...
router.get("/insights", async (req, res) => {
  try {
    ...
    const prompt = `You are the AI of "Bond" — a friendship companion.
${user.name} and ${friend.name} bonded ${bondDays} days ago.
${memories.length} memories, ${bucket.filter(b=>b.completed).length}/${bucket.length} bucket done, avg mood ${avgMood||"N/A"}/5.
